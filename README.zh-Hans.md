...
- 侧栏面板（文件管理器、版本控制、笔记、AI 助手、SSH 连接、端口）以图标栏停靠在窗口左右两侧，把图标拖到另一边就能搬过去
- 标签页里的每个面板共用同一条标题栏，关闭按钮不再浮在内容上
- 终端与编辑器的标题栏以面包屑显示路径，点任一段可浏览该层文件夹，终端直接 cd、编辑器直接换文件，SSH 面板也适用
...
- 同一个 repo 可同时打开多个 worktree，各自独立目录、各自跑 agent
- 从终端的 ⋯ 菜单，或 git 提交图里分支的右键菜单创建 worktree
- 创建时可顺便复制 `.env` 这类本地文件、跑设置命令（每个 repo 各自记住），并直接启动 Claude Code 或 Codex CLI
- 状态栏徽章统计 worktree 数量，点开管理器可查看每个 worktree 的分支、未提交改动、agent 状态与磁盘用量，用完也能从这里移除
- 从管理器打开 worktree：开新标签页或拆分在当前面板旁；已打开的直接跳转
...
- 在同一处浏览 Claude Code、Codex 与 Antigravity CLI 的所有历史对话，直接读取本机的 session 文件，因此不会复制到 TempoTerm 自己的数据库
...
- 实时显示 CPU、内存与网络上下行流量
- 端口：徽章统计监听中的端口，点开端口面板列出每个端口占用的程序与资源使用情况，可直接打开浏览器、打开一个终端到该程序，或结束程序
- Worktree：徽章统计数量，点开就是 worktree 管理器（见上方并行 Worktree）
